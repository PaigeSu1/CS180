...
int main() 
{
    ...
    numberOfCharactersInALine = static_cast<int>((lengthOfALine * POINTS_IN_INCH)/ charPointSize);

    if (lineSpacing == 's' || lineSpacing == 'S')
        numberOfLines = static_cast<int>((length - topMargin - bottomMargin) * POINTS_IN_INCH / charPointSize);
    else if (lineSpacing == 'd' || lineSpacing == 'D')
        numberOfLines = static_cast<int>((length - topMargin - bottomMargin) * POINTS_IN_INCH / charPointSize / 2);
    else
